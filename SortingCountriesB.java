...
public class SortingCountriesB 
{
  public SortingCountriesB()
  {}
  ...
  for (int i = 0 ; i < countryNames.length - 1 ; i++)
  {
      ...
      for (int x = i + 1 ; x < countryNames.length ; x++)
      {
        if (countryNames [x].compareTo(countryNames [alphabeticallyFirst])>0)
        {
          alphabeticallyFirst = x;
        }
      }
      //swaps the maximum element with the first element
      String temp = countryNames [alphabeticallyFirst];  //declares string array and stores second (maximum) value of swap in temporary string array
      String temp2 = capitalCities [alphabeticallyFirst];
      String temp3 = area [alphabeticallyFirst];
      String temp4 = population [alphabeticallyFirst];
      countryNames [alphabeticallyFirst] = countryNames [i]; //swaps first value into second (maximum) value position
      countryNames [i] = temp; //swaps second (maximum) value from temp into the first value position
      capitalCities [alphabeticallyFirst] = capitalCities [i]; //swaps first value into second (maximum) value position
      capitalCities [i] = temp2; //swaps second (maximum) value from temp into the first value position
      area [alphabeticallyFirst] = area [i]; //swaps first value into second (maximum) value position
      area [i] = temp3; //swaps second (maximum) value from temp into the first value position
      population [alphabeticallyFirst] = population [i]; //swaps first value into second (maximum) value position
      population [i] = temp4; //swaps second (maximum) value from temp into the first value position
  }
  ...
  {
    SortingCountriesB test = new SortingCountriesB ();
