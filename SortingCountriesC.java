...
public class SortingCountriesC 
{
  public SortingCountriesC()
  {
  }
  ...
  for (int i = 0 ; i < population.length - 1 ; i++)
  {
      ...
      for (int x = i + 1 ; x < population.length ; x++)
      {
       if (Integer.parseInt (population[x].replace (",",""))>Integer.parseInt(population[maxIndex].replace (",","")))
       {
         maxIndex = x;
       }
      }
      //swaps the maximum element with the first element
      //declares string array and stores second (maximum) value of swap in temporary string array
      String temp = countryNames [maxIndex];  
      String temp2 = capitalCities [maxIndex];
      String temp3 = area [maxIndex];
      String temp4 = population [maxIndex]; 
      countryNames [maxIndex] = countryNames [i]; //swaps first value into second (maximum) value position
      countryNames [i] = temp; //swaps second (maximum) value from temp into the first value position
      capitalCities [maxIndex] = capitalCities [i]; //swaps first value into second (maximum) value position
      capitalCities [i] = temp2; //swaps second (maximum) value from temp into the first value position
      area [maxIndex] = area [i]; //swaps first value into second (maximum) value position
      area [i] = temp3; //swaps second (maximum) value from temp into the first value position
      population [maxIndex] = population [i]; //swaps first value into second (maximum) value position
  ...
  {
    SortingCountriesC test = new SortingCountriesC ();
